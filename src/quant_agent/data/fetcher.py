...
def _fmp(path: str, params: dict | None = None) -> list | dict:
    """GET a single FMP stable endpoint, attach apikey, return parsed JSON.

    Raises ``RuntimeError`` if the key is missing, ``requests.HTTPError`` on
    non-2xx, and ``ValueError`` if FMP returns an error payload.
    """
    if not FMP_API_KEY:
        raise RuntimeError(
...
def _first(raw: list | dict) -> dict:
    """Return the first element of a list-or-single-dict FMP response."""
...
def _fetch_price_history(ticker: str) -> pd.DataFrame:
    """Daily split-adjusted OHLCV from FMP, DatetimeIndex, standard column names.

    FMP's stable EOD endpoint returns split-adjusted ``close`` prices
    (verified by cross-checking through NVDA's June 2024 10:1 split).
    We use a calendar-day date range rather than a ``limit`` param since
    the latter is not consistently honoured by the stable API.
    """
    to_date   = date.today()
    from_date = to_date - timedelta(days=int(PRICE_HISTORY_YEARS * 365 + 60))  # small buffer
    ...
    df = df.set_index("date").sort_index()   # FMP returns newest-first → ascending

    # Rename to yfinance-compatible column names used throughout the pipeline
    df = df.rename(columns={
        "open":   "Open",
        "high":   "High",
        "low":    "Low",
        "close":  "Close",   # already split-adjusted
        "volume": "Volume",
    })
...
def fetch_fundamentals(ticker: str, as_of: date) -> Fundamentals:
    ...
    f = _build_fundamentals(ticker)
    frame = pd.DataFrame([f.model_dump()])
    cache.put(key, as_of, frame)
    return f


def _build_fundamentals(ticker: str) -> Fundamentals:
    """Fetch profile + three financial statements, return a populated Fundamentals."""

    # ── Five calls (each retries internally) ──────────────────────────────── #
    profile_raw = _with_retry(
        lambda: _fmp("/profile", {"symbol": ticker}),
        f"profile[{ticker}]",
    )
    income_raw = _with_retry(
        lambda: _fmp("/income-statement",
    ...
    analyst_target = _fetch_analyst_target(ticker)   # optional; degrades gracefully

    # ── Unpack each response ────────────────────────────────────────────────── #
    ...
    # ── Derived ratios (avoids extra key-metrics / ratios API call) ─────────── #
    ...
    # FMP stable profile: `lastDividend` is the trailing-12-month dividend per
    # share in dollars. Dividing by `price` gives the annualised yield as a
    # decimal that matches the yfinance `dividendYield` contract.
    ...
    return Fundamentals(
        ...
        # ── Prior-period figures (Piotroski year-over-year deltas) ─────────── #
...
def _fetch_analyst_target(ticker: str) -> float | None:
    """FMP price-target consensus (single dict). Returns None on any failure."""
    try:
        raw = _with_retry(
        ...
        logger.debug(f"analyst-target[{ticker}] unavailable: {exc}")
        return None


def _compute_fcf_cagr(cash_statements: list[dict]) -> float | None:
    """Trailing FCF CAGR from up to 4 annual cash flow statements.

    Statements arrive newest-first from FMP (index 0 = most recent period).
    Returns None if FCF is missing or non-positive at either endpoint.
    """
    if not cash_statements:
        return None
...
# --------------------------------------------------------------------------- #
# Options  (graceful fallback — may be unavailable on this FMP plan)
# --------------------------------------------------------------------------- #

def fetch_options(ticker: str, as_of: date) -> OptionsChain | None:
    """Fetch nearest N expirations with full strike ladder (bid/ask/IV).

    FMP's options endpoint may require a higher plan tier. Any failure —
    HTTP error, empty payload, or malformed data — degrades gracefully to None
    so the rest of the pipeline runs unaffected (Black-Scholes signal skipped).
    """
    ...
    try:
        ...
        if not options_list:
            logger.info(f"No options chain available for {ticker} via FMP (plan may not include options)")
            return None

        # Collect nearest N expiration dates
        expirations = sorted({
